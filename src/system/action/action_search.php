...
function staffSearchMember(){
	...
	return boomCode(1, array('data'=> '<div class="page_element">' . $list_members . '</div>'));	
}
...
function staffMoreCritera(){
	...
	if($getmembers->num_rows > 0){
		...
		return boomCode(1, array('data'=> $list_members));
	}
	else {
		return boomCode(0);
	}
...
function staffSearchCritera(){
	...
	return boomCode(1, array('data'=> $list));
}
...
if(isset($_POST['search_member']) && boomAllow(70)){
	echo staffSearchMember();
	die();
}
if(isset($_POST['search_critera']) && boomAllow(70)){
	echo staffSearchCritera();
	die();
}
if(isset($_POST['search_action']) && boomAllow(70)){
	echo staffSearchAction();
	die();
}
if(isset($_POST['more_search_critera'], $_POST['last_critera']) && boomAllow(70)){
